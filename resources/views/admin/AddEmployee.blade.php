    ...
    <main class="container mt-5">
        ...
        <div class="row mt-5">
            <div class="col-lg-5 col-md-12">
                ...
                @endif
                <form action="{{ route('addemployee_post') }}" method="post">
                    @csrf <!-- {{ csrf_field() }} -->
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                      ...
                      <select class="form-select" name="position" id="position">
                        ...
                        <option value="{{ \Auth::user()->role }}">
                            {{ ucwords(Str::replace('_', ' ', \Auth::user()->role))  }}
                        </option>
                        @else
                            <option value="{{ old('position') }}">
                                @if (old('position'))
                                    {{ ucwords(Str::replace('_', ' ', old('position')))  }}
                                @else
                                    Select Position
                                @endif
                                </option>
